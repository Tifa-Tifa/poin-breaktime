...
async function readEmployeeStartDates() {
  ...
  if (!employeeError) {
    const starts = Object.fromEntries(
      (employees || [])
        .filter(row => row.employment_start_date)
        .map(row => [row.id, row.employment_start_date]),
    );
  ...
  // Kompatibilitas sementara selama migrasi database belum dijalankan.
  // Setelah kolom tersedia, jalur mahal ini tidak lagi digunakan.
...
async function writeEntryBatch({ batch, entries, updatedEntries = [], auditEntry }) {
  if (!supabaseAdmin) throw new Error('SUPABASE_SERVICE_ROLE_KEY wajib tersedia untuk menyimpan perubahan.');
  const payload = {
    p_batch:batch,
    p_entries:entries,
    p_updated_entries:updatedEntries.map(tableDefinitions.entries.toRow),
    p_audit:auditEntry || null,
  };
  const { error } = await supabaseAdmin.rpc('save_point_entry_batch', payload);
  if (error) {
    const missingRpc = error.code === 'PGRST202' || /save_point_entry_batch/i.test(error.message || '') && /schema cache|function/i.test(error.message || '');
    if (!missingRpc) throw new Error(`Gagal menyimpan transaksi poin: ${error.message}`);
    // Fallback aman selama RPC belum dipasang: pertahankan urutan foreign key.
    await upsertRows(tableDefinitions.batches.table, [tableDefinitions.batches.toRow(batch)]);
    await Promise.all([
      entries.length ? upsertRows(tableDefinitions.entries.table, entries.map(tableDefinitions.entries.toRow)) : Promise.resolve(),
      updatedEntries.length ? upsertRows(tableDefinitions.entries.table, updatedEntries.map(tableDefinitions.entries.toRow)) : Promise.resolve(),
      auditEntry ? upsertRows(tableDefinitions.audit.table, [tableDefinitions.audit.toRow(auditEntry)]) : Promise.resolve(),
    ]);
  }
  readCache.clear();
  for (const entry of entries) {
    const year = entry.date?.slice(0, 4);
    if (cachedYearRange && year && !cachedYearRange.includes(year)) cachedYearRange.push(year);
    if (cachedEmployeeStarts && entry.status !== 'VOID' && (!cachedEmployeeStarts[entry.employeeId] || entry.date < cachedEmployeeStarts[entry.employeeId])) cachedEmployeeStarts[entry.employeeId] = entry.date;
  }
}
...
module.exports = { readDb, writeDb, writeEntryBatch, readEntryYearRange, readEmployeeStartDates };
