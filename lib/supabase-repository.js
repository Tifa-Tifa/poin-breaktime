...
async function fetchAll(table, configure = query => query, limit = Infinity) {
  ...
  for (let from = 0; ; from += PAGE_SIZE) {
    ...
    rows.push(...data);
    if (data.length < PAGE_SIZE || rows.length >= limit) return rows.slice(0, limit);
  }
}
...
async function readDb(options = {}) {
  ...
  if (useCache && cachedDb && Date.now() - cachedAt < READ_CACHE_MS) return cloneDb(cachedDb);
  if (useCache && pendingRead) return cloneDb(await pendingRead);
  const omitted = new Set(options.omit || ['batches']);
  ...
  if (useCache) { cachedDb = db; cachedAt = Date.now(); }
  return db;
  };
  if (!useCache) return load();
  pendingRead = load();
  try { return cloneDb(await pendingRead); } finally { pendingRead = null; }
}
