...
function entryPage() {
  ...
  return shell(`${pageHead('Input Poin Harian','Pilih kartu aktivitas, lalu tentukan satu atau beberapa karyawan.',`<div class="filter-group"><input class="filter" id="entry-date" type="date" value="${date}"><button class="button" data-page="activity">Riwayat</button></div>`)}
    <section class="day-off-panel"><div><span>STATUS KEHADIRAN</span><h2>Tandai Karyawan OFF</h2><p>Semua poin harian otomatis pada tanggal ini akan dihentikan.</p></div><div class="day-off-input"><input id="day-off-search" list="day-off-options" placeholder="Ketik nama karyawan…" autocomplete="off"><datalist id="day-off-options">${state.employees.filter(e=>e.status==='ACTIVE'&&e.position!=='KAPTEN'&&!offToday.some(x=>x.employeeId===e.id)).map(e=>`<option value="${e.name}" label="${e.position}"></option>`).join('')}</datalist><button class="button" id="save-day-off">Tandai OFF</button></div><div class="day-off-list">${offToday.map(item=>`<span>${item.employee.name}<button data-restore-off="${item.id}">Pulihkan</button></span>`).join('')||'<small>Belum ada karyawan OFF pada tanggal ini.</small>'}</div></section>
...
function bind(){
  ...
  document.querySelector('#entry-date')?.addEventListener('change',e=>{state.entryDate=e.target.value;render()});
